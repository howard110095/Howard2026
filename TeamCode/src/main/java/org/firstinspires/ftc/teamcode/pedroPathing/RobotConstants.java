package org.firstinspires.ftc.teamcode.pedroPathing;

import com.pedropathing.geometry.Pose;

public class RobotConstants {
    public static double shootingTime = 2.5;
    public static int AprilTagNumber = 21;
    public static double AutoVelocity = 3650,toReadyShootingTime = 0.6;



    //blue pose
    public static Pose shootingBluePose = new Pose(-12, 12, Math.toRadians(180));
    public static Pose startBluePose = new Pose(-50.85, 51.33, Math.toRadians(135));
    public static Pose blueRoll1 = new Pose(-53, 10.5, Math.toRadians(180));
    public static Pose blueControl2 = new Pose(-12, -12, Math.toRadians(180));
    public static Pose blueRoll2 = new Pose(-53, -12, Math.toRadians(180));
    public static Pose blueControl3 = new Pose(-12, -36, Math.toRadians(180));
    public static Pose blueRoll3 = new Pose(-53, -36, Math.toRadians(180));



    public static Pose blueLongShootingPose = new Pose(-15, -57.5, Math.toRadians(180));
//    public static Pose startBlueLongPose = new Pose(-24, -60, Math.toRadians(135));
    public static Pose startBlueLongPose = new Pose(-15, -57.5, Math.toRadians(90));
//    public static Pose startBlueLongPose = new Pose(-15, -57.5, Math.toRadians(180));
    public static Pose blueRoll3_2 = new Pose(-55, -36, Math.toRadians(180));
    public static Pose blueCorner = new Pose(-58, -58.5, Math.toRadians(180));


    //red pose

    public static Pose control = new Pose(-15, -36, Math.toRadians(180));












    public static Pose s0_0 = new Pose(0, 0, Math.toRadians(0));
    public static Pose s0_1 = new Pose(0, 50, Math.toRadians(0));
}
